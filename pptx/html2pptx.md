...
// Use SKILL_DIR environment variable (set by skills connector)
...
### Option 2: Generated Scripts (For Charts, Images, and Dynamic Content)

**⚠️ IMPORTANT FOR AGENTS: The CLI wrapper (Option 1) cannot add charts or images. When you need dynamic content, generate a custom JS script using the library API below, then run it via run_script.**

#### 🤖 Recommended Agent Workflow:

1. **Create HTML slide(s)** with `class="placeholder"` elements for chart/image areas
2. **Generate a custom JS file** that uses the html2pptx library API
3. **Run via run_script** and check exitCode for success/failure

#### Example: Agent-Generated Script with Chart

```javascript
// Generated by agent: create-presentation.js
const pptxgen = require('pptxgenjs');
const path = require('path');
...
async function main() {
  try {
    ...
    const { slide, placeholders } = await html2pptx(
      path.join(__dirname, 'slide1.html'),
      pptx
    );

    console.log(`✓ HTML converted, ${placeholders.length} placeholder(s) found`);

    // Add chart to placeholder
    const chartArea = placeholders.find(p => p.id === 'sales-chart');
    if (chartArea) {
    ...
    // Save
    await pptx.writeFile({ fileName: path.join(__dirname, 'output.pptx') });
...
Then run via: `{ "skill": "pptx", "script": "create-presentation.js", "args": [] }`

---

### Option 3: Library API Reference
